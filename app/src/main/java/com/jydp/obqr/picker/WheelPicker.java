	...
	protected void onDraw(Canvas canvas) {
		...
		for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1;
			...
			if(mDataFormat != null) {
				...
				if(data.getClass().isAssignableFrom(SeatListEntity.DataBean.class)) {
					drawText = getDataFormat2(data);
				}else if(data.getClass().isAssignableFrom(StaffEntity.DataBean.class)){
					drawText = getDataFormat3(data);
				} else{
...
//                    computeDistanceToEndPoint(mScroller.getFinalY() % mItemHeight));
	...
	public String getDataFormat2(T data) {
		return ((SeatListEntity.DataBean)data).getNumber();
	}

	public String getDataFormat3(T data) {
		return ((StaffEntity.DataBean)data).getNickname();
	}
